...
import { Box, Flex, Heading, Text, SimpleGrid, Button, HStack, VStack, Icon, useColorModeValue, Badge, Input, InputGroup, InputLeftElement, Menu, MenuButton, MenuList, MenuItem, IconButton, Table, Thead, Tbody, Tr, Th, Td, Tooltip, Modal, ModalOverlay, ModalContent, ModalHeader, ModalFooter, ModalBody, ModalCloseButton, useDisclosure, Tabs, TabList, TabPanels, Tab, TabPanel, Stat, StatLabel, StatNumber, StatHelpText, StatArrow, Progress } from '@chakra-ui/react';
import { FiTrendingUp, FiTrendingDown, FiDroplet, FiDollarSign, FiActivity, FiPlus, FiSearch, FiFilter, FiChevronDown, FiExternalLink, FiLock, FiUnlock, FiInfo, FiStar } from 'react-icons/fi';
    ...
    <Box>
      ...
      <SimpleGrid columns={{ base: 1, md: 3 }} spacing={6} mb={8}>
        ...
        >
          ...
          <Heading size="2xl" mb={2}>$25.3M</Heading>
          <HStack>
        ...
        <Box
          ...
          borderColor={borderColor}
        >
          <Heading size="md" mb={4}>Top APY Pools</Heading>
          <Box h="150px">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                layout="vertical"
                    ...
                    <HStack>
                      <HStack spacing={-1}>
                        {pool.assets.map((asset, index) => (
                          <Box
                            key={index}
                            bg={asset === 'ETH' ? 'blue.500' : asset === 'WBTC' ? 'orange.500' : asset === 'USDC' ? 'green.500' : 'purple.500'}
                            borderRadius="full"
                            w="30px"
                            h="30px"
                            display="flex"
                      ...
                      <VStack align="start" spacing={0}>
                        ...
                        <Text fontSize="xs" color={subTextColor}>
                          {pool.weights.map((w, i) => `${w}% ${pool.assets[i]}`).join(' / ')}
                        </Text>
                      </VStack>
                      {pool.type === 'stable' && (
                        <Badge colorScheme="purple" ml={2}>Stable</Badge>
                      )}
                    </HStack>
                  </Td>
                  <Td isNumeric fontWeight="medium">{pool.tvl}</Td>
                    ...
                    <Box h="200px">
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          />
                          <Line type="monotone" dataKey="tvl" stroke="#0080ff" strokeWidth={2} dot={{ r: 4 }} />
                        </LineChart>
                      </ResponsiveContainer>
                    </Box>
                  </Box>
                  ...
                  <Box>
                    ...
                    <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
                      ...
                      <Box>
                        ...
                        <Text fontWeight="medium" textTransform="capitalize">{selectedPool?.type}</Text>
                      </Box>
                      <Box>
