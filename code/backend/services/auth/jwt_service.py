...
class EnhancedJWTService:
    ...
    def __init__(self):
        ...
        self.access_token_ttl = timedelta(minutes=settings.security.ACCESS_TOKEN_EXPIRE_MINUTES)
        self.refresh_token_ttl = timedelta(days=settings.security.REFRESH_TOKEN_EXPIRE_DAYS)
    ...
    async def create_token_pair(self, user_id: str, roles: List[str], 
                              permissions: List[str], device_info: Optional[DeviceInfo] = None,
                              mfa_verified: bool = False, 
                              custom_claims: Optional[Dict[str, Any]] = None) -> Tuple[str, str]:
    ...
    async def validate_token(self, token: str, expected_type: Optional[TokenType] = None,
                           device_info: Optional[DeviceInfo] = None) -> TokenValidationResult:
        ...
        try:
            ...
            remaining_ttl = int((claims.expires_at - datetime.now(timezone.utc)).total_seconds())
            requires_refresh = remaining_ttl < 300  # Refresh if less than 5 minutes remaining
    ...
    async def refresh_token(self, refresh_token: str, 
                          device_info: Optional[DeviceInfo] = None) -> Tuple[str, str]:
        ...
        logger.info(f"Refreshed token for user {claims.user_id}, session {claims.session_id}")
    ...
    async def revoke_token(self, token: str, reason: str = "Manual revocation"):
        ...
        try:
            ...
            logger.info(f"Revoked token {claims.jti} for user {claims.user_id}: {reason}")
    ...
    async def revoke_user_sessions(self, user_id: str, reason: str = "User logout"):
        ...
        logger.info(f"Revoked {len(sessions_to_revoke)} sessions for user {user_id}: {reason}")
    
    async def create_special_token(self, token_type: TokenType, user_id: str,
    ...
    async def _generate_token(self, claims: TokenClaims) -> str:
        ...
        if claims.token_type in [TokenType.RESET_PASSWORD, TokenType.EMAIL_VERIFICATION]:
            encrypted_token = self.encryption_service.encrypt_token(
                {'token': token}, expires_in=int((claims.expires_at - claims.issued_at).total_seconds())
            )
    ...
    def _parse_token_claims(self, payload: Dict[str, Any]) -> TokenClaims:
        ...
        return TokenClaims(
            ...
            custom_claims={k: v for k, v in payload.items() 
                          if k not in ['sub', 'sid', 'typ', 'roles', 'permissions', 
                                     'device_id', 'ip', 'mfa', 'iat', 'exp', 'nbf', 
                                     'iss', 'aud', 'jti']}
        )
    ...
    async def _validate_device_binding(self, claims: TokenClaims, 
        ...
        if claims.device_id != device_info.device_id:
            logger.warning(f"Device ID mismatch: expected {claims.device_id}, got {device_info.device_id}")
            return False
    ...
    async def _validate_session(self, session_id: str, token_jti: str) -> bool:
        ...
        return (token_jti == session_data.get('access_token_jti') or 
                token_jti == session_data.get('refresh_token_jti'))
    ...
    async def _revoke_session(self, session_id: str, reason: str):
        ...
        if session_data:
            ...
            if 'access_token_jti' in session_data:
                await self._blacklist_token_by_jti(session_data['access_token_jti'], reason)
            if 'refresh_token_jti' in session_data:
                await self._blacklist_token_by_jti(session_data['refresh_token_jti'], reason)
        ...
        if self.redis_client:
            try:
                ...
                try:
                    ...
                    jti = payload.get('jti')
                    if jti:
                        jti_result = await self.redis_client.exists(f"blacklist:jti:{jti}")
