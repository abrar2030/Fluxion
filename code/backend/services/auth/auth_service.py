...
from models.user import User, UserStatus, UserSession, UserActivity
from models.compliance import AuditLog, AuditLogLevel
from schemas.auth import UserRegister, UserLogin, TokenResponse, PasswordChange
...
class AuthService:
    ...
    async def register_user(
        ...
        try:
            ...
            await self._log_user_activity(
                db, user.id, None, "user_registration",
                f"User registered with email {user_data.email}",
                ip_address, user_agent
            )
            
            await self._create_audit_log(
                db, user.id, "user_registration", "User Registration",
    ...
    async def authenticate_user(
        ...
        try:
            ...
            if not user:
                await self._log_failed_login(db, login_data.email, "user_not_found", ip_address, user_agent)
                raise AuthenticationError("Invalid credentials")
            
            # Check if user account is locked
            if user.is_locked():
                await self._log_failed_login(db, login_data.email, "account_locked", ip_address, user_agent)
            ...
            await self._log_user_activity(
                db, user.id, session.id, "user_login",
                "User logged in successfully", ip_address, user_agent
            )
            
            await self._create_audit_log(
                db, user.id, "user_login", "User Login",
                f"User logged in: {user.email}", ip_address, user_agent
            )
    ...
    async def refresh_token(
        ...
        try:
            ...
            # Log token refresh
            await self._log_user_activity(
                db, user.id, session.id, "token_refresh",
                "Access token refreshed", ip_address, user_agent
            )
    ...
    async def logout_user(
        ...
        try:
            ...
            # Log logout
            await self._log_user_activity(
                db, user_id, session_id, "user_logout",
                "User logged out", ip_address, user_agent
            )
            
            await self._create_audit_log(
                db, user_id, "user_logout", "User Logout",
                "User logged out", ip_address, user_agent
            )
    ...
    async def change_password(
        ...
        try:
            ...
            await self.session_service.invalidate_user_sessions(db, user_id, exclude_current=True)
            
            # Log password change
            await self._log_user_activity(
                db, user_id, None, "password_change",
                "Password changed successfully", ip_address, user_agent
            )
            
            await self._create_audit_log(
                db, user_id, "password_change", "Password Change",
                "User changed password", ip_address, user_agent,
    ...
    async def verify_email(
        ...
        try:
            ...
            await self._log_user_activity(
                db, user.id, None, "email_verification",
                "Email verified successfully", ip_address, user_agent
            )
            
            await self._create_audit_log(
                db, user.id, "email_verification", "Email Verification",
                f"Email verified: {user.email}", ip_address, user_agent
            )
    ...
    async def _handle_failed_login(
        ...
        await self._log_failed_login(db, user.email, "invalid_credentials", ip_address, user_agent)
    ...
    async def _log_failed_login(
        ...
        await self._create_audit_log(
            db, None, "failed_login", "Failed Login Attempt",
            f"Failed login attempt for {email}: {reason}",
            ip_address, user_agent, level=AuditLogLevel.WARNING
        )
