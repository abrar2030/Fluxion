...
class PortfolioService:
    ...
    def __init__(self):
        ...
        self.performance_calculation_periods = ['1D', '1W', '1M', '3M', '6M', '1Y', '3Y', '5Y']
    ...
    async def create_portfolio(self, user_id: str, name: str, description: str,
                             portfolio_type: PortfolioType, risk_profile: RiskProfile,
                             target_allocation: Optional[Dict[str, Decimal]] = None) -> Dict[str, Any]:
    ...
    async def get_portfolio(self, portfolio_id: str, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            'target_allocation': {k: str(v) for k, v in portfolio.target_allocation.items()},
            'current_allocation': {k: str(v) for k, v in portfolio.current_allocation.items()},
    ...
    async def add_position(self, portfolio_id: str, asset_symbol: str, quantity: Decimal,
                         purchase_price: Decimal, purchase_date: Optional[datetime] = None) -> Dict[str, Any]:
        ...
        if existing_position:
            ...
            total_cost = (existing_position.quantity * existing_position.average_cost) + (quantity * purchase_price)
        ...
        logger.info(f"Position added to portfolio {portfolio_id}: {asset_symbol} x {quantity}")
        
        return {
            'position_id': position_id,
            'asset_symbol': asset_symbol,
            'quantity': str(quantity),
            'average_cost': str(existing_position.average_cost if existing_position else purchase_price),
            'market_value': str(existing_position.market_value if existing_position else quantity * purchase_price)
        }
    
    async def get_portfolio_positions(self, portfolio_id: str, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        for position in portfolio.positions:
            ...
            if asset:
                formatted_positions.append({
                    ...
                    'unrealized_pnl_percentage': str((position.unrealized_pnl / position.cost_basis * 100).quantize(Decimal('0.01'))),
    ...
    async def calculate_performance_metrics(self, portfolio_id: str, period: str = '1Y') -> Dict[str, Any]:
        ...
        historical_values = await self._get_portfolio_historical_values(portfolio_id, period)
        ...
        excess_returns = [r - float(self.risk_free_rate) / 252 for r in returns]  # Daily risk-free rate
        sharpe_ratio = (
            Decimal(str(statistics.mean(excess_returns))) / volatility * Decimal('15.87')  # sqrt(252)
            if volatility > 0 else Decimal('0')
        )
        
        # Downside deviation for Sortino ratio
        negative_returns = [r for r in returns if r < 0]
        downside_deviation = Decimal(str(statistics.stdev(negative_returns))) if len(negative_returns) > 1 else Decimal('0')
        sortino_ratio = (
            Decimal(str(statistics.mean(excess_returns))) / downside_deviation * Decimal('15.87')
            if downside_deviation > 0 else Decimal('0')
        )
        ...
        var_95 = Decimal(str(statistics.quantiles(returns, n=20)[0])) if len(returns) >= 20 else Decimal('0')
        tail_returns = [r for r in returns if r <= float(var_95)]
        cvar_95 = Decimal(str(statistics.mean(tail_returns))) if tail_returns else Decimal('0')
        ...
        logger.info(f"Performance metrics calculated for portfolio {portfolio_id}, period {period}")
        
        return {
            'portfolio_id': portfolio_id,
            'period': period,
            'total_return': str(metrics.total_return),
            'total_return_percentage': str((metrics.total_return * 100).quantize(Decimal('0.01'))),
            'annualized_return': str(metrics.annualized_return),
            'annualized_return_percentage': str((metrics.annualized_return * 100).quantize(Decimal('0.01'))),
            'volatility': str(metrics.volatility),
            'volatility_percentage': str((metrics.volatility * 100).quantize(Decimal('0.01'))),
            'sharpe_ratio': str(metrics.sharpe_ratio),
            'sortino_ratio': str(metrics.sortino_ratio),
            'max_drawdown': str(metrics.max_drawdown),
            'max_drawdown_percentage': str((metrics.max_drawdown * 100).quantize(Decimal('0.01'))),
    ...
    async def get_rebalancing_recommendations(self, portfolio_id: str) -> Dict[str, Any]:
        ...
        estimated_cost = len(recommended_trades) * Decimal('9.99')  # Simplified commission
        estimated_benefit = sum(allocation_drifts[cat]['drift'] for cat in allocation_drifts) * total_value * Decimal('0.01')
        ...
        return {
            ...
            'cost_benefit_ratio': str((estimated_benefit / estimated_cost).quantize(Decimal('0.01'))) if estimated_cost > 0 else 'N/A',
    ...
    async def _update_portfolio_values(self, portfolio: Portfolio):
        ...
        for position in portfolio.positions:
            ...
            position.current_price = position.average_cost * Decimal('1.05')  # 5% gain simulation
        ...
        portfolio.total_return_percentage = (
            ...
            if invested_amount > 0 else Decimal('0.00')
        )
        ...
        if total_value > 0:
            ...
            if portfolio.cash_balance > 0:
                portfolio.current_allocation['cash'] = portfolio.cash_balance / total_value
        
        # Update position weights
        for position in portfolio.positions:
            position.weight = position.market_value / total_value if total_value > 0 else Decimal('0.00')
    ...
    async def _get_portfolio_historical_values(self, portfolio_id: str, period: str) -> List[Decimal]:
        """Get historical portfolio values (simulated)"""
        # In production, this would query historical data
        # For now, simulate some historical values
        days = {'1D': 1, '1W': 7, '1M': 30, '3M': 90, '6M': 180, '1Y': 365}.get(period, 365)
    ...
    def get_portfolio_statistics(self) -> Dict[str, Any]:
        ...
        for portfolio in self.portfolios.values():
            type_counts[portfolio.portfolio_type.value] = type_counts.get(portfolio.portfolio_type.value, 0) + 1
            risk_profile_counts[portfolio.risk_profile.value] = risk_profile_counts.get(portfolio.risk_profile.value, 0) + 1
