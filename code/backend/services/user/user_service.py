...
class UserService:
    ...
    async def register_user(self, email: str, password: str, user_type: UserType = UserType.INDIVIDUAL,
                          profile_data: Optional[Dict[str, Any]] = None,
                          device_info: Optional[DeviceInfo] = None) -> Dict[str, Any]:
        """Register a new user"""
        ...
        profile = UserProfile(
            ...
            country_of_residence=profile_data.get('country_of_residence', 'US') if profile_data else 'US',
        ...
        preferences = UserPreferences(
            user_id=user_id,
            notification_preferences=self.default_preferences['notification_preferences'].copy(),
            ...
            security_preferences=self.default_preferences['security_preferences'].copy(),
            updated_at=datetime.now(timezone.utc)
        )
    ...
    async def authenticate_user(self, email: str, password: str,
        ...
        if (user.security.account_locked_until and 
            datetime.now(timezone.utc) < user.security.account_locked_until):
            remaining_time = user.security.account_locked_until - datetime.now(timezone.utc)
            raise ValueError(f"Account is locked. Try again in {remaining_time.total_seconds():.0f} seconds")
    ...
    async def update_user_profile(self, user_id: str, updates: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Validate updates
        allowed_fields = {
            'first_name', 'last_name', 'phone_number', 'country_of_residence',
    ...
    async def update_user_preferences(self, user_id: str, preferences: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'trading_preferences' in preferences:
            user.preferences.trading_preferences.update(preferences['trading_preferences'])
        
        if 'display_preferences' in preferences:
            user.preferences.display_preferences.update(preferences['display_preferences'])
        
        if 'security_preferences' in preferences:
            user.preferences.security_preferences.update(preferences['security_preferences'])
    ...
    async def change_password(self, user_id: str, current_password: str, new_password: str) -> Dict[str, Any]:
        ...
        if not password_validation['valid']:
            raise ValueError(f"New password validation failed: {', '.join(password_validation['errors'])}")
        
        # Hash new password
        new_password_hash, new_password_salt = self.encryption_service.hash_password(new_password)
    ...
    async def suspend_user(self, user_id: str, reason: str, admin_user_id: str) -> Dict[str, Any]:
        ...
        # Revoke all sessions
        await self.jwt_service.revoke_user_sessions(user_id, f"Account suspended: {reason}")
    ...
    async def _record_login(self, user: User, device_info: Optional[DeviceInfo], success: bool):
    ...
    def get_user_statistics(self) -> Dict[str, Any]:
        ...
        for user in self.users.values():
            ...
            type_counts[user.user_type.value] = type_counts.get(user.user_type.value, 0) + 1
        
        return {
            'total_users': len(self.users),
            'status_distribution': status_counts,
            'type_distribution': type_counts,
            'mfa_enabled_count': sum(1 for user in self.users.values() if user.security.mfa_enabled)
        }
