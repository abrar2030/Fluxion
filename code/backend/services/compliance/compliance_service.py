...
from models.compliance import (
    KYCRecord, KYCStatus, AMLCheck, AMLRiskLevel, 
    ComplianceAlert, ComplianceAlertType, ComplianceAlertStatus,
    AuditLog, AuditLogLevel
)
from models.risk import RiskProfile, RiskLevel
...
class ComplianceService:
    ...
    def __init__(self):
        ...
        self.high_risk_countries = ['AF', 'IR', 'KP', 'SY']  # Example high-risk countries
    ...
    async def check_transaction_compliance(
        ...
        try:
            ...
            if transaction.usd_value and transaction.usd_value >= self.transaction_threshold_usd:
            ...
            if compliance_result['risk_score'] >= 70:
                compliance_result['compliant'] = False
                compliance_result['recommendations'].append('Transaction requires manual review')
            elif compliance_result['risk_score'] >= 50:
                compliance_result['recommendations'].append('Enhanced monitoring required')
            
            # Create compliance alert if necessary
            if not compliance_result['compliant'] or compliance_result['risk_score'] >= 50:
                await self._create_compliance_alert(
                    db, user.id, transaction.id, compliance_result
                )
            
            logger.info(f"Compliance check completed for transaction {transaction.id}: "
    ...
    async def monitor_user_activity(
        ...
        try:
            ...
            round_number_count = sum(1 for tx in transactions 
                                   if tx.amount and float(tx.amount) % 1000 == 0)
            ...
            structuring_count = sum(1 for tx in transactions 
                                  if tx.usd_value and 
                                  9000 <= tx.usd_value <= 9999)
            ...
            if monitoring_result['risk_score'] >= 30:
                monitoring_result['recommendations'].append('Increased monitoring frequency')
            
            return monitoring_result
            
        except Exception as e:
            logger.error(f"User activity monitoring failed for user {user_id}: {str(e)}")
            raise
        ...
        try:
            ...
            tx_result = await db.execute(
                select(
                    ...
                    func.count(Transaction.id).filter(
                        Transaction.usd_value >= self.transaction_threshold_usd
            ...
            kyc_result = await db.execute(
                select(
                    ...
                    func.count(KYCRecord.id).filter(
            ...
            report['metrics']['kyc'] = {
                ...
                'approval_rate': (kyc_metrics.approved_kyc or 0) / max(kyc_metrics.total_kyc or 1, 1) * 100
            }
            ...
            aml_result = await db.execute(
                select(
                    ...
                    func.count(AMLCheck.id).filter(
                        AMLCheck.risk_level == AMLRiskLevel.HIGH
                    ).label('high_risk_checks'),
                    func.count(AMLCheck.id).filter(
                        or_(
                            AMLCheck.sanctions_match == True,
                            AMLCheck.pep_match == True
                        )
                    ).label('matches_found')
                )
                .where(
            ...
            alert_result = await db.execute(
                select(
                    ...
                    func.count(ComplianceAlert.id).filter(
                        ComplianceAlert.status == ComplianceAlertStatus.OPEN
                    ).label('open_alerts'),
                    func.count(ComplianceAlert.id).filter(
                        ComplianceAlert.status == ComplianceAlertStatus.RESOLVED
            ...
            report['alerts'] = {
                ...
                'resolution_rate': (alert_metrics.resolved_alerts or 0) / max(alert_metrics.total_alerts or 1, 1) * 100
            }
            
            # Generate summary
            report['summary'] = {
                'total_users_monitored': await self._count_active_users(db, start_date, end_date),
                'compliance_score': await self._calculate_compliance_score(report['metrics']),
    ...
    async def _get_user_hourly_transaction_count(self, db: AsyncSession, user_id: UUID) -> int:
    ...
    async def _check_kyc_compliance(self, db: AsyncSession, user: User) -> Dict[str, Any]:
    ...
    async def _check_sanctions(
        ...
        if latest_aml:
            ...
            if latest_aml.adverse_media:
                ...
                sanctions_result['alerts'].append({
                    'type': 'adverse_media',
                    'message': 'Adverse media found for user'
                })
    ...
    async def _analyze_transaction_patterns(self, transactions: List[Transaction]) -> List[Dict[str, Any]]:
        ...
        for i in range(1, len(transactions)):
            diff = (transactions[i-1].created_at - transactions[i].created_at).total_seconds() / 60
    ...
    async def _count_active_users(self, db: AsyncSession, start_date: datetime, end_date: datetime) -> int:
    ...
    def _generate_key_findings(self, metrics: Dict[str, Any]) -> List[str]:
        ...
        if tx_metrics.get('high_value_count', 0) > 0:
            findings.append(f"{tx_metrics['high_value_count']} high-value transactions detected")
        
        kyc_metrics = metrics.get('kyc', {})
        if kyc_metrics.get('approval_rate', 100) < 90:
            findings.append(f"KYC approval rate below 90%: {kyc_metrics['approval_rate']:.1f}%")
    ...
    async def _generate_compliance_recommendations(self, report: Dict[str, Any]) -> List[str]:
        ...
        if open_alerts > 0:
            recommendations.append(f"Review and resolve {open_alerts} open compliance alerts")
