...
class TransactionService:
    ...
    async def create_transaction(self, user_id: str, transaction_type: TransactionType,
        ...
        if not kyc_status['verified'] and transaction_type in [TransactionType.WITHDRAWAL, TransactionType.TRANSFER]:
        ...
        limit_check = await self._check_transaction_limits(user_id, transaction_type, amount, currency)
        ...
        net_amount = amount - fee_amount if transaction_type == TransactionType.WITHDRAWAL else amount
        
        # Check balance for withdrawals and transfers
        if transaction_type in [TransactionType.WITHDRAWAL, TransactionType.TRANSFER]:
            balance_check = await self._check_balance_availability(user_id, currency, amount + fee_amount)
        ...
        transaction_details = TransactionDetails(
            description=details.get('description', f'{transaction_type.value.title()} transaction'),
        ...
        initial_status = self._determine_initial_status(risk_assessment, compliance_checks)
        ...
        await self._add_audit_entry(transaction, 'transaction_created', {
            'created_by': user_id,
    ...
    async def get_transaction(self, transaction_id: str, user_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def get_user_transactions(self, user_id: str, limit: int = 50, 
        ...
        # Apply filters
        if status:
            user_transactions = [tx for tx in user_transactions if tx.status.value == status]
        
        if transaction_type:
            user_transactions = [tx for tx in user_transactions if tx.transaction_type.value == transaction_type]
    ...
    async def get_user_balance(self, user_id: str, currency: Currency) -> Dict[str, Any]:
    ...
    async def cancel_transaction(self, transaction_id: str, user_id: str, reason: str) -> Dict[str, Any]:
        ...
        if transaction.status not in [TransactionStatus.PENDING, TransactionStatus.ON_HOLD]:
            raise ValueError(f"Cannot cancel transaction with status: {transaction.status.value}")
        ...
        if transaction.transaction_type in [TransactionType.WITHDRAWAL, TransactionType.TRANSFER]:
            await self._release_reserved_balance(
                user_id, transaction.currency, transaction.amount + transaction.fee_amount
            )
        
        # Add audit entry
        await self._add_audit_entry(transaction, 'transaction_cancelled', {
    ...
    async def _check_transaction_limits(self, user_id: str, transaction_type: TransactionType,
                                      amount: Decimal, currency: Currency) -> Dict[str, Any]:
        ...
        for limit in user_limits:
            if (limit.transaction_type == transaction_type and 
                limit.currency == currency and
                limit.effective_date <= datetime.now(timezone.utc) and
                (not limit.expiry_date or limit.expiry_date > datetime.now(timezone.utc))):
        ...
        if not applicable_limit:
            ...
            applicable_limit = TransactionLimit(
                ...
                single_transaction_limit=default_limits.get('single_transaction_limit', Decimal('5000.00')),
                currency=currency,
        ...
        current_usage = await self._calculate_current_usage(user_id, transaction_type, currency)
    ...
    async def _calculate_current_usage(self, user_id: str, transaction_type: TransactionType,
                                     currency: Currency) -> Dict[str, Decimal]:
        ...
        for tx in self.transactions.values():
            if (tx.user_id == user_id and 
                tx.transaction_type == transaction_type and 
                tx.currency == currency and
                tx.status in [TransactionStatus.COMPLETED, TransactionStatus.PROCESSING]):
    ...
    async def _calculate_fee(self, transaction_type: TransactionType, amount: Decimal,
                           payment_method: PaymentMethod) -> Decimal:
        """Calculate transaction fee"""
        fee_config = self.fee_structure.get(transaction_type, {}).get(payment_method, Decimal('0'))
        
        if fee_config < 1:  # Percentage fee
            return (amount * fee_config).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
        else:  # Fixed fee
            return fee_config
    
    async def _check_balance_availability(self, user_id: str, currency: Currency,
                                        required_amount: Decimal) -> Dict[str, Any]:
    ...
    async def _assess_transaction_risk(self, user_id: str, transaction_type: TransactionType,
                                     amount: Decimal, currency: Currency,
                                     details: TransactionDetails) -> Dict[str, Any]:
        ...
        # Counterparty risk
        if details.counterparty and await self._is_high_risk_counterparty(details.counterparty):
            risk_score += 2.5
    ...
    async def _perform_compliance_checks(self, user_id: str, transaction_type: TransactionType,
                                       amount: Decimal, currency: Currency,
                                       details: TransactionDetails) -> Dict[str, Any]:
        """Perform compliance checks"""
        checks = {
        ...
        if amount >= Decimal('5000.00') and details.payment_method == PaymentMethod.CRYPTO:
            checks['sar_required'] = True
        
        return checks
    
    def _determine_initial_status(self, risk_assessment: Dict[str, Any],
                                compliance_checks: Dict[str, Any]) -> TransactionStatus:
    ...
    async def _process_transaction_async(self, transaction_id: str):
        ...
        try:
            ...
            await self._add_audit_entry(transaction, 'processing_completed', {
                'confirmation_number': transaction.confirmation_number
            })
            ...
            if transaction:
                ...
                await self._add_audit_entry(transaction, 'processing_failed', {
                    'error': str(e)
                })
    ...
    async def _update_balance(self, user_id: str, currency: Currency, 
                            amount: Decimal, operation: str):
        """Update user balance"""
        balance_key = (user_id, currency)
        balance = self.balances.get(balance_key)
    ...
    async def _release_reserved_balance(self, user_id: str, currency: Currency, amount: Decimal):
        """Release reserved balance"""
        balance_key = (user_id, currency)
        balance = self.balances.get(balance_key)
    ...
    async def _add_audit_entry(self, transaction: Transaction, action: str, details: Dict[str, Any]):
    ...
    def _estimate_completion_time(self, transaction: Transaction) -> str:
        ...
        if transaction.details.payment_method == PaymentMethod.INTERNAL:
            return "Immediate"
        elif transaction.details.payment_method in [PaymentMethod.ACH, PaymentMethod.BANK_TRANSFER]:
    ...
    def get_transaction_statistics(self) -> Dict[str, Any]:
        ...
        for tx in self.transactions.values():
            ...
            type_counts[tx.transaction_type.value] = type_counts.get(tx.transaction_type.value, 0) + 1
