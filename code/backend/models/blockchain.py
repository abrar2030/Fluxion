...
from sqlalchemy import (
    ...
    Enum, ForeignKey, Index, JSON, DECIMAL, BigInteger
)
...
class BlockchainNetwork(BaseModel, TimestampMixin, AuditMixin):
    ...
    # Network details
    network_type = Column(String(20), nullable=False, comment="Network type (mainnet, testnet)")
    consensus_mechanism = Column(String(20), nullable=True, comment="Consensus mechanism")
    ...
    # Network status
    status = Column(Enum(NetworkStatus), default=NetworkStatus.ACTIVE, nullable=False, comment="Network status")
    is_supported = Column(Boolean, default=True, nullable=False, comment="Network support status")
    
    # Performance metrics
    current_block_number = Column(BigInteger, nullable=True, comment="Current block number")
    last_block_time = Column(DateTime(timezone=True), nullable=True, comment="Last block timestamp")
    average_gas_price = Column(DECIMAL(36, 18), nullable=True, comment="Average gas price")
    
    # Configuration
    gas_limit_multiplier = Column(DECIMAL(5, 2), default=1.2, nullable=False, comment="Gas limit multiplier")
    max_gas_price = Column(DECIMAL(36, 18), nullable=True, comment="Maximum gas price")
    confirmation_blocks = Column(Integer, default=12, nullable=False, comment="Required confirmation blocks")
...
class SmartContract(BaseModel, TimestampMixin, AuditMixin):
    ...
    network_id = Column(UUID(as_uuid=True), ForeignKey("blockchain_networks.id"), nullable=False, comment="Network ID")
    ...
    deployed_at_block = Column(BigInteger, nullable=True, comment="Deployment block number")
    deployed_at_tx = Column(String(66), nullable=True, comment="Deployment transaction hash")
    deployer_address = Column(String(42), nullable=True, comment="Deployer address")
    
    # Contract status
    status = Column(Enum(ContractStatus), default=ContractStatus.ACTIVE, nullable=False, comment="Contract status")
    is_verified = Column(Boolean, default=False, nullable=False, comment="Source code verified")
    is_proxy = Column(Boolean, default=False, nullable=False, comment="Is proxy contract")
    implementation_address = Column(String(42), nullable=True, comment="Implementation contract address")
    
    # Monitoring configuration
    monitor_events = Column(Boolean, default=True, nullable=False, comment="Monitor contract events")
    event_filters = Column(JSON, nullable=True, comment="Event monitoring filters")
    last_processed_block = Column(BigInteger, nullable=True, comment="Last processed block")
    ...
    # Indexes
    __table_args__ = (
        Index('idx_smart_contracts_network_address', 'network_id', 'address', unique=True),
...
class ContractEvent(BaseModel, TimestampMixin):
    ...
    contract_id = Column(UUID(as_uuid=True), ForeignKey("smart_contracts.id"), nullable=False, comment="Contract ID")
    ...
    # Processing status
    status = Column(Enum(EventStatus), default=EventStatus.PENDING, nullable=False, comment="Processing status")
    ...
    # Business logic
    business_event_type = Column(String(50), nullable=True, comment="Business event type")
    related_user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True, comment="Related user ID")
    related_transaction_id = Column(UUID(as_uuid=True), ForeignKey("transactions.id"), nullable=True, comment="Related transaction ID")
...
class SupplyChainAsset(BaseModel, TimestampMixin, AuditMixin):
    ...
    asset_id = Column(BigInteger, unique=True, nullable=False, comment="On-chain asset ID")
    metadata = Column(Text, nullable=False, comment="Asset metadata")
    
    # Current state
    current_custodian = Column(String(42), nullable=False, comment="Current custodian address")
    status = Column(Enum(AssetStatus), default=AssetStatus.CREATED, nullable=False, comment="Asset status")
    location = Column(String(200), nullable=True, comment="Current location")
    
    # Creation details
    creator_address = Column(String(42), nullable=False, comment="Asset creator address")
    creation_tx_hash = Column(String(66), nullable=False, comment="Creation transaction hash")
    creation_block = Column(BigInteger, nullable=False, comment="Creation block number")
    
    # Transfer tracking
    transfer_count = Column(Integer, default=0, nullable=False, comment="Number of transfers")
    last_transfer_at = Column(DateTime(timezone=True), nullable=True, comment="Last transfer timestamp")
    
    # Verification and compliance
    is_verified = Column(Boolean, default=False, nullable=False, comment="Asset verification status")
...
class AssetTransfer(BaseModel, TimestampMixin):
    ...
    asset_id = Column(UUID(as_uuid=True), ForeignKey("supply_chain_assets.id"), nullable=False, comment="Asset ID")
    ...
    transfer_timestamp = Column(DateTime(timezone=True), nullable=False, comment="Transfer timestamp")
    ...
    # Blockchain details
    transaction_hash = Column(String(66), nullable=False, comment="Transfer transaction hash")
    block_number = Column(BigInteger, nullable=False, comment="Block number")
    
    # Proof and verification
    proof_hash = Column(String(66), nullable=True, comment="Transfer proof hash")
    is_verified = Column(Boolean, default=False, nullable=False, comment="Transfer verification status")
...
class LiquidityPool(BaseModel, TimestampMixin, AuditMixin):
    ...
    # Pool identification
    pool_id = Column(BigInteger, unique=True, nullable=False, comment="On-chain pool ID")
    ...
    # Pool status
    is_active = Column(Boolean, default=True, nullable=False, comment="Pool active status")
    is_paused = Column(Boolean, default=False, nullable=False, comment="Pool paused status")
    
    # Liquidity metrics
    total_liquidity = Column(DECIMAL(36, 18), default=0, nullable=False, comment="Total liquidity")
    total_volume = Column(DECIMAL(36, 18), default=0, nullable=False, comment="Total trading volume")
    total_fees = Column(DECIMAL(36, 18), default=0, nullable=False, comment="Total fees collected")
    
    # Performance metrics
    apy = Column(Float, nullable=True, comment="Annual percentage yield")
    daily_volume = Column(DECIMAL(36, 18), nullable=True, comment="Daily trading volume")
    weekly_volume = Column(DECIMAL(36, 18), nullable=True, comment="Weekly trading volume")
    
    # Oracle configuration
    oracles = Column(JSON, nullable=True, comment="Price oracles")
    oracle_heartbeats = Column(JSON, nullable=True, comment="Oracle heartbeat intervals")
...
class SyntheticAsset(BaseModel, TimestampMixin, AuditMixin):
    ...
    collateral_ratio = Column(DECIMAL(10, 8), nullable=False, comment="Collateral ratio")
    
    # Supply information
    total_supply = Column(DECIMAL(36, 18), default=0, nullable=False, comment="Total supply")
    circulating_supply = Column(DECIMAL(36, 18), default=0, nullable=False, comment="Circulating supply")
    ...
    # Status
    is_active = Column(Boolean, default=True, nullable=False, comment="Asset active status")
    is_paused = Column(Boolean, default=False, nullable=False, comment="Asset paused status")
