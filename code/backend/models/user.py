...
from sqlalchemy import (
    ...
    Enum, ForeignKey, Index, JSON, Float
)
...
class User(BaseModel, TimestampMixin, SoftDeleteMixin, AuditMixin, EncryptedMixin, ComplianceMixin):
    ...
    username = Column(String(100), unique=True, nullable=True, index=True, comment="Username")
    ...
    phone_number = Column(String(255), nullable=True, comment="Phone number (encrypted)")
    ...
    status = Column(Enum(UserStatus), default=UserStatus.PENDING, nullable=False, comment="Account status")
    ...
    mfa_enabled = Column(Boolean, default=False, nullable=False, comment="MFA enabled flag")
    ...
    # Security tracking
    failed_login_attempts = Column(Integer, default=0, nullable=False, comment="Failed login attempts count")
    locked_until = Column(DateTime(timezone=True), nullable=True, comment="Account lock expiry")
    ...
    password_changed_at = Column(DateTime(timezone=True), nullable=True, comment="Password change timestamp")
    
    # KYC and compliance
    kyc_status = Column(Enum(KYCStatus), default=KYCStatus.NOT_STARTED, nullable=False, comment="KYC status")
    kyc_completed_at = Column(DateTime(timezone=True), nullable=True, comment="KYC completion timestamp")
    kyc_expires_at = Column(DateTime(timezone=True), nullable=True, comment="KYC expiry timestamp")
    
    # Blockchain wallets
    primary_wallet_address = Column(String(42), nullable=True, index=True, comment="Primary wallet address")
    wallet_addresses = Column(JSON, nullable=True, comment="Additional wallet addresses")
    ...
    currency = Column(String(10), default="USD", nullable=False, comment="Preferred currency")
    
    # Notification preferences
    email_notifications = Column(Boolean, default=True, nullable=False, comment="Email notifications enabled")
    sms_notifications = Column(Boolean, default=False, nullable=False, comment="SMS notifications enabled")
    push_notifications = Column(Boolean, default=True, nullable=False, comment="Push notifications enabled")
    
    # Terms and privacy
    terms_accepted_at = Column(DateTime(timezone=True), nullable=True, comment="Terms acceptance timestamp")
    privacy_accepted_at = Column(DateTime(timezone=True), nullable=True, comment="Privacy policy acceptance timestamp")
...
class UserProfile(BaseModel, TimestampMixin, AuditMixin):
    ...
    annual_income = Column(String(255), nullable=True, comment="Annual income (encrypted)")
    ...
    # Social and referral
    referral_code = Column(String(20), unique=True, nullable=True, comment="User's referral code")
    referred_by = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True, comment="Referring user ID")
    
    # Profile completion
    profile_completion_percentage = Column(Integer, default=0, nullable=False, comment="Profile completion percentage")
    ...
    def calculate_completion_percentage(self) -> int:
        ...
        fields = [
            self.occupation, self.annual_income, self.investment_experience,
            self.risk_tolerance, self.user.first_name, self.user.last_name,
            self.user.phone_number, self.user.address_line1, self.user.city,
            self.user.country
        ]
...
class UserSession(BaseModel, TimestampMixin):
    ...
    session_token = Column(String(255), unique=True, nullable=False, index=True, comment="Session token")
    refresh_token = Column(String(255), unique=True, nullable=True, index=True, comment="Refresh token")
    ...
    expires_at = Column(DateTime(timezone=True), nullable=False, comment="Session expiry")
    last_activity_at = Column(DateTime(timezone=True), nullable=True, comment="Last activity timestamp")
    
    # Security
    is_suspicious = Column(Boolean, default=False, nullable=False, comment="Suspicious activity flag")
...
class UserActivity(BaseModel, TimestampMixin):
    ...
    is_flagged = Column(Boolean, default=False, nullable=False, comment="Flagged for review")
