...
class RiskProfile(BaseModel, TimestampMixin, AuditMixin):
    ...
    # Risk tolerance
    risk_tolerance = Column(Enum(RiskLevel), default=RiskLevel.MEDIUM, nullable=False, comment="Risk tolerance level")
    risk_capacity = Column(Enum(RiskLevel), default=RiskLevel.MEDIUM, nullable=False, comment="Risk capacity level")
    
    # Investment profile
    investment_experience = Column(String(50), nullable=True, comment="Investment experience level")
    investment_horizon = Column(String(50), nullable=True, comment="Investment time horizon")
    ...
    # Risk limits
    max_portfolio_risk = Column(Float, nullable=True, comment="Maximum portfolio risk percentage")
    max_single_asset_allocation = Column(Float, nullable=True, comment="Maximum single asset allocation")
    max_sector_allocation = Column(Float, nullable=True, comment="Maximum sector allocation")
    max_drawdown_tolerance = Column(Float, nullable=True, comment="Maximum drawdown tolerance")
    ...
    monthly_var_limit = Column(DECIMAL(20, 8), nullable=True, comment="Monthly VaR limit")
    
    # Leverage limits
    max_leverage = Column(Float, default=1.0, nullable=False, comment="Maximum leverage ratio")
    margin_requirement = Column(Float, nullable=True, comment="Margin requirement percentage")
    
    # Questionnaire responses
    questionnaire_responses = Column(JSON, nullable=True, comment="Risk questionnaire responses")
    questionnaire_score = Column(Integer, nullable=True, comment="Risk questionnaire score")
    questionnaire_date = Column(DateTime(timezone=True), nullable=True, comment="Questionnaire completion date")
    
    # Profile status
    is_active = Column(Boolean, default=True, nullable=False, comment="Profile active status")
    last_reviewed_at = Column(DateTime(timezone=True), nullable=True, comment="Last review timestamp")
    next_review_due = Column(DateTime(timezone=True), nullable=True, comment="Next review due date")
    
    # Relationships
    user = relationship("User", back_populates="risk_profiles")
    assessments = relationship("RiskAssessment", back_populates="risk_profile", cascade="all, delete-orphan")
    alerts = relationship("RiskAlert", back_populates="risk_profile", cascade="all, delete-orphan")
    ...
    def calculate_risk_score(self) -> int:
        ...
        # Weighted average
        return int((tolerance_score * 30 + capacity_score * 30 + questionnaire_score * 40) / 100)
...
class RiskAssessment(BaseModel, TimestampMixin, AuditMixin):
    ...
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False, comment="User ID")
    portfolio_id = Column(UUID(as_uuid=True), ForeignKey("portfolios.id"), nullable=True, comment="Portfolio ID")
    risk_profile_id = Column(UUID(as_uuid=True), ForeignKey("risk_profiles.id"), nullable=False, comment="Risk profile ID")
    
    # Assessment details
    assessment_type = Column(String(50), nullable=False, comment="Assessment type")
    assessment_date = Column(DateTime(timezone=True), nullable=False, comment="Assessment date")
    
    # Overall risk metrics
    overall_risk_level = Column(Enum(RiskLevel), nullable=False, comment="Overall risk level")
    ...
    operational_risk_score = Column(Float, nullable=True, comment="Operational risk score")
    compliance_risk_score = Column(Float, nullable=True, comment="Compliance risk score")
    ...
    var_confidence_level = Column(Float, default=95.0, nullable=False, comment="VaR confidence level")
    ...
    concentration_risk = Column(Float, nullable=True, comment="Concentration risk score")
    largest_position_weight = Column(Float, nullable=True, comment="Largest position weight")
    top_5_positions_weight = Column(Float, nullable=True, comment="Top 5 positions weight")
    ...
    risk_recommendations = Column(JSON, nullable=True, comment="Risk mitigation recommendations")
    
    # Model information
    model_version = Column(String(20), nullable=True, comment="Risk model version")
    calculation_method = Column(String(50), nullable=True, comment="Calculation methodology")
...
class RiskAlert(BaseModel, TimestampMixin, AuditMixin):
    ...
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False, comment="User ID")
    portfolio_id = Column(UUID(as_uuid=True), ForeignKey("portfolios.id"), nullable=True, comment="Portfolio ID")
    risk_profile_id = Column(UUID(as_uuid=True), ForeignKey("risk_profiles.id"), nullable=True, comment="Risk profile ID")
    risk_assessment_id = Column(UUID(as_uuid=True), ForeignKey("risk_assessments.id"), nullable=True, comment="Risk assessment ID")
    ...
    status = Column(Enum(RiskAlertStatus), default=RiskAlertStatus.ACTIVE, nullable=False, comment="Alert status")
    ...
    recommendation = Column(Text, nullable=True, comment="Risk mitigation recommendation")
    ...
    breach_percentage = Column(Float, nullable=True, comment="Threshold breach percentage")
    ...
    # Response
    acknowledged_by = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True, comment="User who acknowledged")
...
class RiskLimit(BaseModel, TimestampMixin, AuditMixin):
    ...
    warning_threshold = Column(DECIMAL(20, 8), nullable=True, comment="Warning threshold")
    currency = Column(String(10), nullable=True, comment="Currency for monetary limits")
    
    # Limit scope
    asset_symbol = Column(String(20), nullable=True, comment="Asset symbol (for asset-specific limits)")
    asset_category = Column(String(50), nullable=True, comment="Asset category")
    
    # Status
    is_active = Column(Boolean, default=True, nullable=False, comment="Limit active status")
    is_breached = Column(Boolean, default=False, nullable=False, comment="Limit breach status")
    
    # Monitoring
    current_value = Column(DECIMAL(20, 8), nullable=True, comment="Current value")
    utilization_percentage = Column(Float, nullable=True, comment="Limit utilization percentage")
    last_checked_at = Column(DateTime(timezone=True), nullable=True, comment="Last check timestamp")
    
    # Breach information
    first_breach_at = Column(DateTime(timezone=True), nullable=True, comment="First breach timestamp")
    last_breach_at = Column(DateTime(timezone=True), nullable=True, comment="Last breach timestamp")
    breach_count = Column(Integer, default=0, nullable=False, comment="Total breach count")
...
class RiskScenario(BaseModel, TimestampMixin, AuditMixin):
    ...
    # Status
    is_active = Column(Boolean, default=True, nullable=False, comment="Scenario active status")
