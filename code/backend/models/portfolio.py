...
from sqlalchemy import (
    ...
    Enum, ForeignKey, Index, JSON, Numeric, DECIMAL, Float
)
from sqlalchemy.dialects.postgresql import UUID
from sqlalchemy.orm import relationship

from code.backend.models.base import BaseModel, TimestampMixin, AuditMixin, ComplianceMixin
...
class Portfolio(BaseModel, TimestampMixin, AuditMixin, ComplianceMixin):
    ...
    status = Column(Enum(PortfolioStatus), default=PortfolioStatus.ACTIVE, nullable=False, comment="Portfolio status")
    
    # Portfolio type and strategy
    portfolio_type = Column(String(50), default="default", nullable=False, comment="Portfolio type")
    investment_strategy = Column(String(100), nullable=True, comment="Investment strategy")
    risk_level = Column(String(20), nullable=True, comment="Risk level")
    
    # Financial metrics
    total_value_usd = Column(DECIMAL(20, 8), default=0, nullable=False, comment="Total portfolio value in USD")
    total_cost_basis_usd = Column(DECIMAL(20, 8), default=0, nullable=False, comment="Total cost basis in USD")
    unrealized_pnl_usd = Column(DECIMAL(20, 8), default=0, nullable=False, comment="Unrealized P&L in USD")
    realized_pnl_usd = Column(DECIMAL(20, 8), default=0, nullable=False, comment="Realized P&L in USD")
    ...
    # Diversification
    asset_count = Column(Integer, default=0, nullable=False, comment="Number of different assets")
    network_count = Column(Integer, default=0, nullable=False, comment="Number of different networks")
    
    # Timestamps
    last_updated_at = Column(DateTime(timezone=True), nullable=True, comment="Last portfolio update")
    last_rebalanced_at = Column(DateTime(timezone=True), nullable=True, comment="Last rebalancing")
    
    # Settings
    auto_rebalance = Column(Boolean, default=False, nullable=False, comment="Auto-rebalancing enabled")
    rebalance_threshold = Column(Float, default=5.0, nullable=False, comment="Rebalancing threshold percentage")
    
    # Relationships
    user = relationship("User", back_populates="portfolios")
    assets = relationship("PortfolioAsset", back_populates="portfolio", cascade="all, delete-orphan")
    holdings = relationship("AssetHolding", back_populates="portfolio", cascade="all, delete-orphan")
    ...
    def needs_rebalancing(self) -> bool:
        ...
        for asset in self.assets:
            if asset.target_allocation:
                current_allocation = self.calculate_allocation_percentage(asset.asset_symbol)
...
class PortfolioAsset(BaseModel, TimestampMixin, AuditMixin):
    ...
    portfolio_id = Column(UUID(as_uuid=True), ForeignKey("portfolios.id"), nullable=False, comment="Portfolio ID")
    ...
    target_allocation = Column(Float, nullable=True, comment="Target allocation percentage")
    min_allocation = Column(Float, nullable=True, comment="Minimum allocation percentage")
    max_allocation = Column(Float, nullable=True, comment="Maximum allocation percentage")
    
    # Asset status
    status = Column(Enum(AssetStatus), default=AssetStatus.ACTIVE, nullable=False, comment="Asset status")
    is_tracked = Column(Boolean, default=True, nullable=False, comment="Asset tracking enabled")
    ...
    # Unique constraint
    __table_args__ = (
        Index('idx_portfolio_assets_unique', 'portfolio_id', 'asset_symbol', 'contract_address', unique=True),
    )
...
class AssetHolding(BaseModel, TimestampMixin, AuditMixin):
    ...
    available_quantity = Column(DECIMAL(36, 18), nullable=False, comment="Available quantity")
    locked_quantity = Column(DECIMAL(36, 18), default=0, nullable=False, comment="Locked quantity")
    staked_quantity = Column(DECIMAL(36, 18), default=0, nullable=False, comment="Staked quantity")
    
    # Cost basis and valuation
    average_cost_price = Column(DECIMAL(36, 18), nullable=True, comment="Average cost price")
    ...
    current_value_usd = Column(DECIMAL(20, 8), nullable=True, comment="Current value in USD")
    
    # P&L calculations
    unrealized_pnl = Column(DECIMAL(20, 8), nullable=True, comment="Unrealized P&L")
    unrealized_pnl_percentage = Column(Float, nullable=True, comment="Unrealized P&L percentage")
    realized_pnl = Column(DECIMAL(20, 8), default=0, nullable=False, comment="Realized P&L")
    ...
    # Metadata
    last_price_update = Column(DateTime(timezone=True), nullable=True, comment="Last price update")
    ...
    def get_allocation_percentage(self) -> float:
        ...
        if self.portfolio and self.portfolio.total_value_usd > 0 and self.current_value_usd:
            return float((self.current_value_usd / self.portfolio.total_value_usd) * 100)
        return 0.0
...
class PortfolioSnapshot(BaseModel, TimestampMixin):
    ...
    # Snapshot details
    snapshot_date = Column(DateTime(timezone=True), nullable=False, comment="Snapshot date")
    snapshot_type = Column(String(20), nullable=False, comment="Snapshot type (daily, weekly, monthly)")
    
    # Financial metrics
    total_value_usd = Column(DECIMAL(20, 8), nullable=False, comment="Total value in USD")
    total_cost_basis_usd = Column(DECIMAL(20, 8), nullable=False, comment="Total cost basis in USD")
    unrealized_pnl_usd = Column(DECIMAL(20, 8), nullable=False, comment="Unrealized P&L in USD")
    realized_pnl_usd = Column(DECIMAL(20, 8), nullable=False, comment="Realized P&L in USD")
...
class AssetPrice(BaseModel, TimestampMixin):
    ...
    source_timestamp = Column(DateTime(timezone=True), nullable=True, comment="Source timestamp")
