...
class ComplianceValidator:
    ...
    def _validate_terraform_security(self, tf_file: Path) -> None:
        ...
        try:
            ...
            self.warnings.append({
                "category": "terraform",
                "message": f"Could not parse {tf_file}: {e}"
            })
    ...
    def validate_secrets_management(self, base_path: str) -> None:
        ...
        for file_path in base_dir.rglob("*"):
            if file_path.is_file() and file_path.suffix in ['.yml', '.yaml', '.tf', '.py', '.js']:
                try:
    ...
    def print_summary(self) -> None:
        ...
        print(f"Status: {'✅ COMPLIANT' if len(self.violations) == 0 else '❌ NON-COMPLIANT'}")
        print()
        ...
        if self.violations:
            ...
            for violation in self.violations:
                ...
                print(f"  {severity_icon} [{violation['category'].upper()}] {violation['message']}")
            print()
...
def main():
    ...
    parser.add_argument("--kubernetes", default="infrastructure/kubernetes", help="Kubernetes manifests path")
    parser.add_argument("--terraform", default="infrastructure/terraform", help="Terraform configuration path")
    parser.add_argument("--ansible", default="infrastructure/ansible", help="Ansible configuration path")
    parser.add_argument("--docker", default="infrastructure", help="Docker configuration path")
